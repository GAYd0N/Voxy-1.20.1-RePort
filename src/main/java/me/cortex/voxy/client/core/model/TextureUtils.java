...
public class TextureUtils {
    ...
    public static int mipColours(int one, int two, int three, int four) {
        return mipColours(false, one, two, three, four);
    }
    ...
    private static int linearToSrgbChannel(float c) {
        if (c <= 0.0031308f) {
            return (int) (12.92f * c * 255.0f);
        } else {
            return (int) ((1.055f * (float) Math.pow(c, 1.0 / 2.4) - 0.055f) * 255.0f);
        }
    }
