...
public class MipGen {
    ...
    private static void solidify(long baseAddr, byte msk) {
        for (int idx = 0; idx < 6; idx++) {
            ...
            long cAddr = baseAddr + (long) (bx + (long) by * MODEL_TEXTURE_SIZE * 3) * 4;
            Arrays.fill(SCRATCH, (short) -1);
            for (int y = 0; y < MODEL_TEXTURE_SIZE; y++) {
                for (int x = 0; x < MODEL_TEXTURE_SIZE; x++) {
                    int colour = MemoryUtil.memGetInt(cAddr + (x + (long) y * MODEL_TEXTURE_SIZE * 3) * 4);
        ...
        for (int i = 0; i < LAYERS - 1; i++) {
            ...
            for (int px = 0; px < width; px++) {
                for (int py = 0; py < height; py++) {
                    ...
                    MemoryUtil.memPutInt(dAddr + (px + (long) py * width) * 4L, TextureUtils.mipColours(darkened, C00, C01, C10, C11));
                }
            }
        }
    }
