...
public class WorldConversionFactory {
    ...
    private static boolean setupCanaryLocalPallet(Palette<BlockState> vp,
            Reference2IntOpenHashMap<BlockState> blockCache, Mapper mapper, int[] pc) {
    ...
    private static void setupLocalPalette(Palette<BlockState> vp, Reference2IntOpenHashMap<BlockState> blockCache,
        ...
        if (vp instanceof LinearPalette<BlockState>) {
            ...
            if (!handled) {
                throw new IllegalStateException(
                        "Unknown palette type: " + vp + " (class: " + vp.getClass().getName() + ")");
            }
        }
    }
    ...
    public static void mipSection(VoxelizedSection section, Mapper mapper) {
        ...
        for (int y = 0; y < 8; y += 2) {
            for (int z = 0; z < 8; z += 2) {
                for (int x = 0; x < 8; x += 2) {
                    data[16 * 16 * 16 + 8 * 8 * 8 + i++] = Mipper.mip(
                            data[H(x, y, z)], data[H(x + 1, y, z)], data[H(x, y, z + 1)], data[H(x + 1, y, z + 1)],
                            data[H(x, y + 1, z)], data[H(x + 1, y + 1, z)], data[H(x, y + 1, z + 1)],
                            data[H(x + 1, y + 1, z + 1)],
                            mapper);
                }
            }
        ...
        for (int y = 0; y < 4; y += 2) {
            for (int z = 0; z < 4; z += 2) {
                for (int x = 0; x < 4; x += 2) {
                    data[16 * 16 * 16 + 8 * 8 * 8 + 4 * 4 * 4 + i++] = Mipper.mip(
                            data[I(x, y, z)], data[I(x + 1, y, z)], data[I(x, y, z + 1)], data[I(x + 1, y, z + 1)],
                            data[I(x, y + 1, z)], data[I(x + 1, y + 1, z)], data[I(x, y + 1, z + 1)],
                            data[I(x + 1, y + 1, z + 1)],
                            mapper);
                }
            }
