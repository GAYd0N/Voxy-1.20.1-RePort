...
public class VoxelIngestService {
    ...
    private static byte[] acquireLightDataFromDataLayer(DataLayer dataLayer) {
        if (dataLayer == null) return null;
        return acquireLightData(dataLayer.getData());
    }
    ...
    // 改为普通类以支持 Mixin 接口注入（Ecliptic-Seasons 兼容）
    public static class IngestSection {
        public final int cx, cy, cz;
        public final WorldEngine world;
        public final PalettedContainer<BlockState> states;
        public final PalettedContainerRO<Holder<Biome>> biomes;
        public final boolean onlyAir;
        public final byte[] blockLight;
        public final byte[] skyLight;

        public IngestSection(int cx, int cy, int cz, WorldEngine world,
                             PalettedContainer<BlockState> states,
                             PalettedContainerRO<Holder<Biome>> biomes,
                             boolean onlyAir, byte[] blockLight, byte[] skyLight) {
            this.cx = cx;
            this.cy = cy;
            this.cz = cz;
            this.world = world;
    ...
    private void processJob() {
        ...
        if (task.onlyAir && task.blockLight == null && task.skyLight == null) {
            ...
            ILightingSupplier lightSupplier = createLightingSupplier(task.blockLight, task.skyLight);
    ...
    @NotNull
    private static ILightingSupplier createLightingSupplier(byte[] blockLight, byte[] skyLight) {
        if (blockLight == null && skyLight == null) {
            return (x, y, z) -> 0;
        }

        return new ILightingSupplier() {
            @Override
            public byte supply(int x, int y, int z) {
                int idx = (y << 8) | (z << 4) | x;
                int halfIdx = idx >> 1;
                boolean shift = (idx & 1) != 0;

                int sky = 0;
                if (skyLight != null) {
                    int b = skyLight[halfIdx] & 0xFF;
                    sky = shift ? (b >> 4) : (b & 0xF);
                }

                int block = 0;
                if (blockLight != null) {
                    int b = blockLight[halfIdx] & 0xFF;
    ...
    private static IngestSection snapshotSection(int cx, int cy, int cz, WorldEngine world,
    ...
    public boolean enqueueIngest(WorldEngine engine, LevelChunk chunk) {
        ...
        for (var section : chunk.getSections()) {
            ...
            if (lightingProvider.getDebugSectionType(LightLayer.SKY, pos) != LayerLightSectionStorage.SectionType.LIGHT_AND_DATA
                    && lightingProvider.getDebugSectionType(LightLayer.BLOCK, pos) != LayerLightSectionStorage.SectionType.LIGHT_AND_DATA)
                continue;
            gotLighting = true;
        }
        ...
        for (var section : chunk.getSections()) {
            ...
            byte[] blData = acquireLightDataFromDataLayer(bl);

            var sl = slp.getDataLayerData(pos);
            byte[] slData = acquireLightDataFromDataLayer(sl);
