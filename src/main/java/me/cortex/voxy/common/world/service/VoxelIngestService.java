...
public class VoxelIngestService {
    ...
    private void processJob() {
        ...
        if (section.hasOnlyAir() && task.blockLight == null && task.skyLight == null) {// If the chunk section has
                                                                                       // lighting data, propagate it
    ...
    public boolean enqueueIngest(WorldEngine engine, LevelChunk chunk) {
        ...
        if (allEmpty && !gotLighting) {
            ...
            for (var section : chunk.getSections()) {
                ...
                if (section == null || !shouldIngestSection(section, chunk.getPos().x, i, chunk.getPos().z))
                    continue;
                this.ingestQueue
                        .add(new IngestSection(chunk.getPos().x, i, chunk.getPos().z, engine, section, null, null));
                try {
        ...
        for (var section : chunk.getSections()) {
            ...
            this.ingestQueue.add(new IngestSection(chunk.getPos().x, i, chunk.getPos().z, engine, section, bl, sl));// TODO:
                                                                                                                    // fixme,
                                                                                                                    // this
                                                                                                                    // is
                                                                                                                    // technically
    ...
    private boolean rawIngest0(WorldEngine engine, LevelChunkSection section, int x, int y, int z, DataLayer bl,
            DataLayer sl) {
